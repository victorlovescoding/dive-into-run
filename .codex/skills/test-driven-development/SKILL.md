...
_ **Action 1 (Read The Laws)**: 在撰寫任何測試代碼前，**必須依序執行以下指令**，否則視為任務失敗：1. `Read` 讀取 `.codex/skills/test-driven-development/references/coding-style.md` (這是你的法律，必須嚴格遵守)。2. `Read` 讀取 `.codex/skills/test-driven-development/references/jsdoc-cheatsheet.md` (這是你的語法字典，請查閱型別寫法)。3. `Read` 讀取 `.codex/skills/test-driven-development/references/boilerplate.js` (這是你的範本，請照抄結構)。
_ **Action 2 (Anti-Patterns)**: `Read` 讀取並理解 `.codex/skills/test-driven-development/references/testing-anti-patterns.md`。**Iron Rule**: 嚴格遵守「三不原則」：不測試 Mock 行為、不污染生產代碼、不使用不完整 Mock。
...
_ **Legacy note**: 舊 feature-local test folders 只作為 git history / legacy provenance，不是新增或修改測試的目標。`specs/<feature>/` 只保留 `spec.md` / `plan.md` / `tasks.md` 等 planning artifacts，不得建立 test/test directories。
_ **Action**: 新增測試一律寫入 repo-root `tests/`：1. **Layout**: - Unit → `tests/unit/<layer>/<name>.test.js[x]`（layer = service/repo/runtime/lib/config）- Integration → `tests/integration/<domain>/<name>.test.jsx` - E2E → `tests/e2e/<name>.spec.js` - Server → `tests/server/<suite>/<name>.test.js`（g8 server coverage 使用 `tests/server/g8-server-coverage/`）2. **Create Directories**（首次寫入時，子資料夾不存在才需要建）：
`             mkdir -p tests/unit/<layer> tests/integration/<domain> tests/e2e
            ...
            `
_ **Result**: 輸出你選定的 unit / integration / e2e / server 測試檔絕對路徑與 `tests/test-results/[unit|integration|e2e]/` 結果路徑供使用者確認。

2.7 **建立 Stub 檔案 (Stub Creation)**: - **Why**: Vite/Vitest 在跑測試時會解析 import 路徑；被 import 的檔案不存在會直接 crash，無法得到有效 RED。- **When**: 在撰寫測試（Step 3）**之前**。根據 `plan.md` 的「新增檔案清單」，為尚未存在的 source 檔案建立 stub。- **Workflow**: Stub 只保留 export signature、完整 JSDoc 與 `Not implemented`/零值，不寫任何邏輯；需要具體格式時讀 `.codex/skills/test-driven-development/references/boilerplate.js` 與 `.codex/references/testing-handbook.md`，不要把長範例內嵌在 skill。
...
3.  **撰寫測試 (Testing)**:
    ...
    - **Unit Tests**:
      - **Target**: `src/service/**` 的純 business/service logic、`src/repo/**` 的 data adapters、`src/runtime/**` 的 hooks/use-cases；`src/lib/**` 只用於 compatibility facade contract 測試。
      - **Path**: `tests/unit/<layer>/`。
...
> **Command**: `git add <explicit-test-file-path> <explicit-stub-file-path-if-created> && git commit -m "test($(git branch --show-current)): add failing tests (RED)"`
