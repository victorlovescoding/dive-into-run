...
2.  **驗收測試**:
    - **準備**: 確保已建立 `specs/$(git branch --show-current)/verification/` 目錄。
    - **對照標準**: 以 `specs/$(git branch --show-current)/spec.md` 的 **User Stories** 為唯一真理。
    - **執行動作**: 每條 User Story 必須有對應的 MCP 操作與截圖。
    - **截圖路徑**: 存放於 `specs/$(git branch --show-current)/verification/`。
    - **驗收清單**:
        1. 核心功能是否正常運作
        2. UI 是否符合設計
...
3.  **紀錄驗收結果**:
    - **Action**: 在 `specs/$(git branch --show-current)/verification.md` 紀錄審查結果。
    - **內容規範**:
...
5.  **標記狀態**:
    - **若驗收通過**: 在 `specs/$(git branch --show-current)/tasks.md` **Frontmatter (檔案最開頭)** 加入（或更新）標記：`status: verified`。進入 Step 9。
    - **若驗收失敗**: **禁止標記**。
