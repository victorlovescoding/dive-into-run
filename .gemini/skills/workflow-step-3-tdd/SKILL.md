...
5.  **驗證測試 (Red)**:
    ...
    *   **Commands**:
        - Unit: `npx vitest run tests/$(git branch --show-current)/unit`
        - Integration: `npx vitest run tests/$(git branch --show-current)/integration`
        - E2E: `npx playwright test tests/$(git branch --show-current)/e2e`
