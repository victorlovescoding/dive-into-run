...
> 新增 integration test 一律放 `tests/integration/<domain>/`；Phase 2 完成後舊 `specs/<feature>/tests/` 下的 `integration/` 已清空，僅 git history 保留追溯。

```
tests/
├── unit/                            # 已 Phase 1 搬遷（依 domain 分組）
└── integration/                     # Phase 2 進行中（依 domain 分組）
    ├── notifications/               # 例：tests/integration/notifications/NotificationBell.test.jsx
    ├── posts/                       # 例：tests/integration/posts/feed.test.jsx
    ├── events/                      # 例：tests/integration/events/event-form.test.jsx
    └── <domain>/                    # 一律放 tests/integration/<domain>/X.test.jsx
...
└── <feature>/                       # <feature> = git branch 名稱
    ...
    ├── tests/
    │   └── e2e/                     # *.spec.js（Phase 3 規劃中，仍保留於 specs/）
    └── test-results/
        ├── unit/results.xml
...
**檔案位置**：新增測試一律放 `tests/integration/<domain>/X.test.jsx`（例 `tests/integration/notifications/`、`tests/integration/posts/`）。Phase 2 並存期間舊位置仍可讀，但禁止再新增。
