...
2.7 **建立 Stub 檔案 (Stub Creation)**: - **Why**: Vite/Vitest 在跑測試時會解析 import 路徑；被 import 的檔案不存在會直接 crash，無法得到有效 RED。- **When**: 在撰寫測試（Step 3）**之前**。根據 `plan.md` 的「新增檔案清單」，為尚未存在的 source 檔案建立 stub。- **Workflow**: Stub 只保留 export signature、完整 JSDoc 與 `Not implemented`/零值，不寫任何邏輯；需要具體格式時讀 `.claude/skills/test-driven-development/references/boilerplate.js` 與 `.claude/references/testing-handbook.md`，不要把長範例內嵌在 skill。
