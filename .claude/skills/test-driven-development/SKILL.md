...
_ **並存期說明**: 若分支已有 `specs/<branch>/tests/`，新檔仍可放舊位置直到該 spec 進入 Phase 1-3 遷移；新分支一律走 repo-root `tests/`。
_ **Action**: 新增測試一律寫入 repo-root `tests/`：1. **Layout**: - Unit → `tests/unit/<layer>/<name>.test.js`（layer = service/repo/runtime/lib/config）- Integration → `tests/integration/<domain>/<name>.test.jsx` - E2E → `tests/e2e/<name>.spec.js` 2. **Create Directories**（首次寫入時，子資料夾不存在才需要建）：
`             mkdir -p tests/{unit,integration,e2e}
            `
\_ **Result**: 輸出你選定的測試檔絕對路徑供使用者確認。
...
3.  **撰寫測試 (Testing)**:
    - **Parallel Strategy**: 三個層級的測試**可以平行撰寫**（用 3 個 subagent 各負責一份）。若平行執行：
      ...
      - 每個 subagent **應參考前一個 feature 同層級的測試檔案**作為 pattern reference（用 `Glob` 找 `tests/unit/**/*.test.js` 等取最近的範本），確保 mock scaffold、import pattern、JSDoc style 一致。
    - **Requirement**: 必須**明確處理**以下三個層級的測試。對於每一個層級，**必須**建立測試檔案，**或**明確說明「為何本任務不需要此層級測試」(例如：純 UI 修改不涉邏輯則免 Unit Test)。
    - **Unit Tests**:
      - **Target**: `src/service/` 的純商業邏輯、`src/repo/` 的資料存取邏輯；`src/lib/` 為 compatibility facade，僅在需驗證 re-export 契約時補測。
...
4.  **The Iron Wall (絕對防線 - Zero Tolerance)**:
    ...
    - **Variable Definition**: 下列指令中的 `$TEST_FILE_PATH` 代表**你剛剛建立的測試檔案路徑** (e.g. `tests/unit/<layer>/login.test.jsx`)。
...
5.  **驗證測試 (Red)**:
    ...
    - **Commands**:
      - Unit: `mkdir -p tests/test-results/unit && npx vitest run tests/unit/<layer>/<file> --reporter=junit --outputFile=tests/test-results/unit/results.xml`
      - Integration: `mkdir -p tests/test-results/integration && npx vitest run tests/integration/<domain>/<file> --reporter=junit --outputFile=tests/test-results/integration/results.xml`
      - E2E: `mkdir -p tests/test-results/e2e && PLAYWRIGHT_JUNIT_OUTPUT_NAME=tests/test-results/e2e/results.xml npx playwright test tests/e2e/<file> --reporter=junit`
    - **Test Results (Output)**:
