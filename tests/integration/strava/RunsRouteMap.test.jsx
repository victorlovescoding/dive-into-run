...
vi.mock('react-leaflet', () => ({
  MapContainer: ({ children }) => <div role="region" aria-label="路線地圖">{children}</div>,
  TileLayer: () => null,
  Polyline: ({ positions }) => (
...
describe('RunsRouteMap', () => {
  it('renders MapContainer with decoded coordinates as Polyline using real decoder', () => {
    ...
    expect(screen.getByRole('region', { name: '路線地圖' })).toBeInTheDocument();
  ...
  it('renders nothing when summaryPolyline is null', () => {
    ...
    expect(screen.queryByRole('region', { name: '路線地圖' })).not.toBeInTheDocument();
