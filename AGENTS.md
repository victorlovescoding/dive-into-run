...
2. **Task Profile Routing** — `docs/superpowers/task-profiles.md`
   ...
   - P1/P2 default to a branch and no `specs/` artifacts.
   - P1/P2 upgrade to a worktree when the workspace is occupied by an active branch/task, dirty tree, or unmerged PR.
   - P3/P4 follow `docs/superpowers/task-profiles.md` for worktree and `specs/` artifact rules.
   - P1/P2/P3 的輕量 profile 只減少 artifacts/ceremony；repo-changing edits 仍先交給 Engineer subagent，main agent 不直接改 production code、tests、docs 或其他 repo files。
   - 非 read-only 的 repo-changing work 預設要有 Reviewer subagent check；純 read-only exploration 可用 read-only subagent，通常不需要 Reviewer。
   - 輕量 profile 不得繞過 branch/worktree、commit、push feature branch、PR、required `ci` + `e2e` green、GitHub merge、本地 `main` fast-forward、owned files/non-scope、fresh verification、protected `main` 規則。
...
7. **TDD 實作** — `subagent-driven-development`
   - 對 development、bugfix、refactor、testing、docs 與其他 repo-changing work，主 agent 預設只做 dispatcher/coordinator；不得直接改 production code、tests、docs 或其他 repo files，實際 edits 先交給 Engineer subagent。
   ...
   - Main agent is control plane only. It may read `AGENTS.md`,
     ...
     `git status --short --branch`, task-local diffs/changed-file lists, and
     exact Engineer/Reviewer evidence lines. It must not do broad source
     exploration, design fixes from its own code investigation, or replace
     Engineer/Reviewer with self-review.
   - Pure exploration goes to a bounded read-only Explorer subagent with
     file/line/command/uncertainty evidence.
   - Docs-only, workflow docs, ADR, `.codex/**`, scripts, and config changes
     are still repo-changing work: default Engineer-owned edit plus Reviewer
     check. The only main-agent edit exception is workflow state updates that
     record dispatch, review result, blocker, or closeout evidence.
8. **Review / Debug** — paired Reviewer + `receiving-code-review` / `systematic-debugging`
   - 非 read-only 的 repo-changing work 預設由 Reviewer subagent 檢查；Reviewer PASS 前不得勾選 task；Reviewer REJECT 退回同一 Engineer 修到 PASS。
   - `tasks.md`、`status.json`、`handoff.md` 有 drift 時，不得 dispatch、
     commit、push、開 PR、merge 或 sync local `main`；先 reconcile 或 block。
     Reviewer PASS 不可由 main-agent self-check 取代。
9. **驗證與收尾** — `verification-before-completion` → `finishing-a-development-branch`
   ...
   - `lastVerification` 與 command evidence 一筆一 command；不要把 `&&` 或
     `;` 串成同一筆。subagent narrative 只能當 hint，必須有 raw command
     summary、file:line evidence 或 reviewed diff。
   - P1/P2/P3 若沒有 approved `spec.md`，必須明確記錄授權邊界到 edit、
     commit、push、PR、merge、local `main` sync 的哪一步；「開始修」不等於
     merge/sync 授權。
   - Main agent closeout 可 stage/commit/push/PR/watch CI/sync main，但只能
     在 reviewed diff 後且授權邊界允許時做。Staging 必須列具體檔案，不得
     建議或執行 `git add .`、`git add -A`、`git add --all`。
   - 預設 closeout：branch/worktree → commit → push feature branch → 開 PR → required `ci` + `e2e` green → GitHub merge PR → 本地 `main` fast-forward 到 `origin/main`。
