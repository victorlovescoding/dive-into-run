...
- `queryEvents` mixes Firestore queries (Stage 1) and in-memory filtering (Stage 2).
  - _Critique_: While this scales poorly (O(N) on the client side after fetching), for an MVP with limited event volume, it avoids the nightmare of Firestore Composite Indexes. It is a pragmatic "Worse is Better" choice.
  - _Warning_: If `limit(50)` is hit by 50 events that are all filtered out by distance, the user sees nothing despite valid events existing further down the list. This is a known trade-off of this pattern.
