...
2. **Given** 一筆跑步紀錄，**When** 顯示在列表中，**Then** 卡片上方為活動名稱與活動日期，下方橫排顯示距離(km)、配速(min/km)、總時間三項指標
...
- **FR-006**: 每筆跑步紀錄必須顯示：活動名稱、活動日期、距離(km)、配速(min/km)、總時間，三項指標橫排呈現
...
- MVP 每次同步（含首次與增量）最多拉取 100 筆活動，不做 Strava API 分頁迭代
