...
- [ ] 等 GitHub protected-branch status checks（lint / test）綠 → merge → 刪 branch
- [ ] **S5 啟動**（`test(rules): firestore rules infra + 5 critical paths`）：先 read `firestore.rules:80-254` 真實規則，再補 `tests/server/rules/{users,posts,strava,events,notifications}.rules.test.js` 與 `.github/workflows/firestore-rules-gate.yml`（audit L614-620）
