...
- [ ] S005 拆 `firebase-events.js` / `firebase-comments.js` / `firebase-member.js` 的 repo vs service vs use-case 責任。
- [ ] S006 拆 `firebase-posts.js` / `firebase-notifications.js` / `firebase-profile.js` 的 repo vs service vs use-case 責任。
...
- [ ] S010 拆 `src/app/events/page.jsx` 為 thin entry + runtime + ui。
- [ ] S011 拆 `src/app/events/[id]/eventDetailClient.jsx` 為 thin entry + runtime + ui。
...
- [ ] S016 加入 `dependency-cruiser` package、config、scripts，完成 ESLint / dep-cruise 分工。
