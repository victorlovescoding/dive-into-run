...
> **前提（gate）**：Phase 0 PR 已 merge + main 跑滿 24h 無 CI 紅。Phase 0 commit `88d3b94` 目前**尚未 push**（見 [`./phase-0-handoff.md`](./phase-0-handoff.md)），Phase 1 不可在 push 前啟動。
> **Spec folder**：`specs/023-tests-directory-migration/`（同 Phase 0）
> **Branch**：`023-tests-directory-migration`（沿用，不另開 sub-branch；commit 接在 88d3b94 之後）
> **預估工期**：1-2 天
> **總任務數**：13（T101-T113）
...
Wave 5 (1 worker-reviewer):                       │
  └─ Agent-Smoke:        T111                    (5 quality gate + Phase 1 specific 驗證)
                                                  │
Wave 6 (1 worker-reviewer):                       │
  └─ Agent-Commit:       T112                    (commit Phase 1)
                                                  │
Wave 7 (manual):                                  │
  └─ User:               T113                    (push + open PR + 24h 觀察)
```
...
## Phase 1 Wave 6 — Commit（1 agent）

### T112 [Engineer-Commit] commit Phase 1

**Dependency**：T111 全綠

**Engineer Prompt** (派 Engineer-Commit, `general-purpose`):

> 你是 Phase 1 commit engineer。
>
> 1. `git status -s` 確認改動範圍（預期）：
>    - `M  vitest.config.mjs`
>    - `M  specs/021-layered-dependency-architecture/test-buckets/policy.js`
>    - `A  specs/023-tests-directory-migration/phase-1-mv-inventory.md`
...
> 2. `git diff --cached --stat | tail -5` 看摘要
> 3. **不要** `git add -A`（會撈到 untracked `project-health/` / `phase-0-handoff.md`）。逐項 add：
>    ```bash
...
>    git add tests/unit/   # 新建 sub-folder + .gitkeep（若有）
>    # 60 個 git mv 已自動 stage，無需重 add
>    ```
> 4. commit 用 heredoc：
>
>    ```bash
>    git commit -m "$(cat <<'EOF'
>    refactor(tests): migrate ~60 unit tests to top-level tests/unit (Phase 1)
>
...
>    - vitest.config.mjs coverage scope 從 src/lib/** 擴成 src/{service,repo,runtime,lib,config}/**
>    - vitest.config.mjs thresholds.lines 暫降 80（觀察期，TODO 提回 95）
>    - policy.js KNOWN_S015_CONFLICTS 8 條 filePath 同步新 tests/unit/ 位置
...
>    ```
>
>    **不加** `Co-Authored-By`（用戶 preference）。
>
> 5. `git log -1 --stat | tail -10` + `git status` 回報結果。
>
> **不要** push（user 親跑 Wave 7）。
>
> **若 pre-commit hook 失敗**：報告失敗訊息，**不重試 commit、不 amend、不 --no-verify**。回報主 agent 派對應 wave engineer 修。

**Acceptance Criteria**:

- [ ] commit 成功（pre-commit hook 全綠：lint + type-check + spellcheck + vitest browser）
- [ ] commit message 含 `Phase 1` 與「migrate ~60 unit tests」字樣
- [ ] commit message **不含** `Co-Authored-By`
- [ ] `git log -1 --stat` 顯示 ≥ 60 檔變動

**Verify Command** (Reviewer-Commit, `Explore` 跑):

```bash
git log -1 --pretty='%s%n%b' | head -20
# 看訊息是否符合
git log -1 --shortstat
# 看檔案數
files_changed=$(git log -1 --pretty=format: --shortstat | grep -oE '[0-9]+ files? changed' | grep -oE '^[0-9]+')
[ -z "$files_changed" ] && { echo "FAIL: no commit detected"; exit 1; }
[ "$files_changed" -ge 60 ] || { echo "FAIL: files_changed=$files_changed expected ≥ 60"; exit 1; }

# 不該有 Co-Author
git log -1 --pretty='%b' | grep -qi 'co-authored-by' && { echo "FAIL: Co-Authored-By found"; exit 1; } || true

# message 含 Phase 1
git log -1 --pretty='%s%n%b' | grep -qi 'phase 1' || { echo "FAIL: no Phase 1 in message"; exit 1; }
...
### T113 [User] push + open PR + 24h 觀察

**Description**：T112 commit 完後 user 親跑（涉及 push 到 remote — destructive 程度需 user confirm）。
...
### Step 6：Wave 6 commit（1 agent）

主 agent 派 Engineer-Commit 跑 T112，配對 Reviewer-Commit 確認。

### Step 7：Wave 7 PR（user manual）

主 agent 通報「Phase 1 commit 完成（sha: <X>），可開 PR」+ 提供 T113 範本給 user 複製貼上。**主 agent 不自己跑 push / gh**。
...
# 確認回到 Phase 0 head
git log -1 --pretty='%h %s'   # 期望：88d3b94 phase 0 quality-gate prep
git status -s                  # 期望：empty + untracked project-health/ + phase-0-handoff.md
```

### Wave 6 commit 後失敗（commit 已下，push 未做）

```bash
# 退 commit 但保留改動
git reset --soft HEAD~1
# 或全砍回 Phase 0 head（destructive，user confirm）
git reset --hard 88d3b94
```
