...
**Why this priority**: 跑者是重度手機使用族群，目前導覽列在手機上 6 個連結 + 登入/登出按鈕會溢出或擠在一起，嚴重影響基本使用體驗。這是最核心的問題。
...
- **FR-006a**: 認證狀態載入期間，導覽列右側必須顯示圓形骨架動畫（skeleton）佔位，避免 layout shift 和 UI 閃爍
...
- **FR-011**: 導覽列必須具備適當的無障礙支援（語義化 HTML、鍵盤導覽、ARIA 標籤）
