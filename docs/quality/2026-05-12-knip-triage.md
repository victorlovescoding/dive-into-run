...
| --- | --- | --- | --- |
| `deferred-follow-up` | unused files: 16 `src/hooks/**` and `src/lib/**` files | `npm run knip:report` reported `Unused files (16)` including `src/hooks/useCommentMutations.js`, `src/hooks/useComments.js`, `src/hooks/useDashboardTab.js`, `src/hooks/useRunCalendar.js`, `src/hooks/useStravaActivities.js`, `src/hooks/useStravaConnection.js`, `src/hooks/useStravaSync.js`, and multiple `src/lib/**` files | Defer; source deletion is broad runtime cleanup and outside Task 1 and Option A cleanup scope. Next owner must prove runtime reachability or remove each file under a separate source-cleanup task. |
| `blocked-unclear` | unused dependencies in the full report: `functions/package.json: firebase-admin` | `npm run knip:report` reported `Unused dependencies (1)` for `functions/package.json: firebase-admin` | Stop before cleanup; package metadata removal cannot be proven from the allowed narrow context. Next owner must validate Functions runtime and deploy-time usage before changing dependency metadata. |
| `blocked-unclear` | production unused dependencies: `functions/package.json: firebase-admin` and `package.json: @mapbox/polyline, firebase, firebase-admin, leaflet-geosearch, react-leaflet, topojson-client` | `npm run knip:production-deps` reported `Unused dependencies (2)` with those package manifests and dependency names | Stop before cleanup; production dependency removal needs runtime and deployment ownership proof outside Task 1. Next owner must split app dependencies from Functions dependencies before proposing package changes. |
| `blocked-unclear` | unused devDependencies: `package.json: eslint-config-next, taiwan-atlas` | `npm run knip:report` reported `Unused devDependencies (1)` with those names | Stop before cleanup; dev dependency metadata removal cannot be proven from the allowed narrow context. Next owner must check lint/config and data-generation entrypoints before removal. |
| `blocked-unclear` | unlisted dependencies: `@next/eslint-plugin-next`, `globals`, `estree`, `@babel/parser`, `geojson` | `npm run knip:report` reported `Unlisted dependencies (5)` in `eslint.config.mjs`, `scripts/audit-use-effect-data-fetching.mjs`, `specs/021-layered-dependency-architecture/test-buckets/policy.js`, `src/components/weather/TaiwanMap.jsx`, and `src/config/geo/weather-geo-cache.js` | Stop before cleanup; deciding whether to add metadata, rewrite imports, or treat transitive/tooling usage as intentional needs files outside the Task 1 write scope and broader ownership judgment. Next owner must classify each import as direct dependency, tooling-only dependency, or intentional transitive usage before editing manifests. |
| `blocked-unclear` | unlisted binaries: `functions/package.json: firebase`; `package.json: firebase, open` | `npm run knip:report` reported `Unlisted binaries (2)` with those binary names | Stop before cleanup; binary usage and package-script intent cannot be proven enough for metadata changes in Task 1. Next owner must verify package scripts, CI usage, and local tooling expectations before adding or removing dependencies. |
| `deferred-follow-up` | unused exports: 23 export groups across specs, UI contexts, repo, runtime, and service modules | `npm run knip:report` reported `Unused exports (23)` including `Navbar.jsx`, context defaults, notification helpers, Strava helpers, repo functions, runtime hooks, providers, and service helpers | Defer; export deletion is broad source/API cleanup and outside Task 1 and Option A cleanup scope. Next owner must handle this as API cleanup with affected tests, not as metadata cleanup. |
| `deferred-follow-up` | duplicate exports: `specs/021-layered-dependency-architecture/test-buckets/policy.js` duplicate export groups | `npm run knip:report` reported `Duplicate exports (2)` for `evaluateTestDependency`, `evaluateTestModuleSpecifier`, `KNOWN_S015_UNIT_CONFLICTS`, and `KNOWN_S015_CONFLICTS` | Defer; specs helper cleanup is not part of Task 1 and needs ownership review. Next owner must confirm the spec helper contract before consolidating exports. |
...
## Follow-Up Ownership And Expiry

| Bucket | Owner note | Expiry / next review trigger |
| --- | --- | --- |
| `blocked-unclear` dependency metadata | Assign to the next dependency-maintenance owner with Functions and app runtime context. Do not batch manifest edits unless each dependency has direct ownership proof. | Re-review by 2026-06-12, or earlier when a package metadata task is opened. |
| `blocked-unclear` unlisted dependencies | Assign to the owner of the importing file or script named in the Knip finding. Each import needs an explicit decision: direct dependency, dev dependency, rewrite, or documented intentional transitive usage. | Re-review by 2026-06-12, or earlier when lint/script/weather/spec helper ownership changes. |
| `blocked-unclear` unlisted binaries | Assign to the package-script and local-tooling owner. Verify CI, emulator, and local developer entrypoints before changing manifests. | Re-review by 2026-06-12, or earlier when package scripts or emulator tooling change. |
| `deferred-follow-up` source files and exports | Assign to the owning runtime/UI/service/spec-helper area. Treat cleanup as source/API work with targeted tests, not as Knip metadata cleanup. | Re-review by 2026-06-12, or earlier when the owning module is refactored. |

Expiry means the triage decision should be revisited; it does not authorize
cleanup without fresh evidence and an owned task.
