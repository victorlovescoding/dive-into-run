...
當**所有需要的測試檔案** (Unit / Integration / E2E / Server) 都已建立且確認為 Red 狀態後，請先列出本次 RED phase 實際新增/修改的具體檔案，再提示使用者執行以下指令（一次性提交）：

> **Stage**: `git add <test-file-paths...> <stub-source-file-paths...>`
> **Commit**: `git commit -m "test(<scope>): add failing tests (RED)"`

禁止使用 `git add .`、`git add -A` 或 `git add --all`。`<scope>` 應由目前分支或 feature 名稱明確填入，不要依賴 command substitution。
