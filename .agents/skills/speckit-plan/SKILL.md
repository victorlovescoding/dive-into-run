---
name: "speckit-plan"
description: "Explicit-only Spec Kit adapter: use only when the user invokes $speckit-plan; execute the implementation planning workflow using the plan template to generate design artifacts."
...
When updating the `<!-- SPECKIT START -->` block, keep its explicit-only
conditional language; it must not force ordinary repo work to read broad
current-plan context or override Context Routing.
